...
static inline float DeterminantM3(Mat3 Matrix)
{
    ...
    Mat3 HHM_Cross;
    HHM_Cross.Columns[0] = Cross(Matrix.Columns[1], Matrix.Columns[2]);
    HHM_Cross.Columns[1] = Cross(Matrix.Columns[2], Matrix.Columns[0]);
    HHM_Cross.Columns[2] = Cross(Matrix.Columns[0], Matrix.Columns[1]);

    return DotV3(HHM_Cross.Columns[2], Matrix.Columns[2]);
}
...
static inline Mat3 InvGeneralM3(Mat3 Matrix)
{
    ...
    float InvDeterminant = 1.0f / DotV3(HHM_Cross.Columns[2], Matrix.Columns[2]);

    Mat3 Result;
    Result.Columns[0] = MulV3F(HHM_Cross.Columns[0], InvDeterminant);
    Result.Columns[1] = MulV3F(HHM_Cross.Columns[1], InvDeterminant);
    Result.Columns[2] = MulV3F(HHM_Cross.Columns[2], InvDeterminant);
...
COVERAGE(NormQ, 1)
static inline Quat NormQ(Quat HMM_Quat)
{
    ASSERT_COVERED(NormQ);

    /* NOTE(lcf): Take advantage of SSE implementation in NormV4 */
    Vec4 Vec = {HMM_Quat.X, HMM_Quat.Y, HMM_Quat.Z, HMM_Quat.W};
    Vec = NormV4(Vec);
...
static inline Quat M4ToQ_RH(Mat4 M)
{
    ...
    HMM_Q = MulQF(HMM_Q, 0.5f / SqrtF(T));
